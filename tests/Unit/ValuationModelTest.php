<?php

namespace Tests\Unit;

use Tests\TestCase;
use App\Models\User;
use App\Models\Valuation;
use App\Models\Currency;
use App\Models\ValuationUpdate;
use App\Models\ValuationHistory;
use App\Enums\ValuationUpdateMethod;
use Illuminate\Foundation\Testing\RefreshDatabase;

class ValuationModelTest extends TestCase
{
    use RefreshDatabase;

    protected function setUp(): void
    {
        parent::setUp();
        // Disable observers for testing
        Valuation::unsetEventDispatcher();
    }

    public function test_get_rate_limiter_key_for_yahoo(): void
    {
        $user = User::factory()->create();
        $currency = Currency::create(['symbol' => 'EUR', 'main' => true, 'user_id' => $user->id]);

        $valuation = Valuation::create([
            'name' => 'AAPL',
            'currency_id' => $currency->id,
            'update_method' => ValuationUpdateMethod::YAHOO,
            'user_id' => $user->id,
        ]);

        $this->assertEquals('yahoo', $valuation->rate_limiter_key);
    }

    public function test_get_rate_limiter_key_for_xpath(): void
    {
        $user = User::factory()->create();
        $currency = Currency::create(['symbol' => 'EUR', 'main' => true, 'user_id' => $user->id]);

        $valuation = Valuation::create([
            'name' => 'TEST',
            'currency_id' => $currency->id,
            'update_method' => ValuationUpdateMethod::XPATH,
            'update_data' => ['url' => 'https://example.com/page'],
            'user_id' => $user->id,
        ]);

        $this->assertEquals('example.com', $valuation->rate_limiter_key);
    }

    public function test_get_rate_limiter_key_for_openai(): void
    {
        $user = User::factory()->create();
        $currency = Currency::create(['symbol' => 'EUR', 'main' => true, 'user_id' => $user->id]);

        $valuation = Valuation::create([
            'name' => 'TEST',
            'currency_id' => $currency->id,
            'update_method' => ValuationUpdateMethod::OPENAI,
            'user_id' => $user->id,
        ]);

        $this->assertEquals('openai', $valuation->rate_limiter_key);
    }

    public function test_get_rate_limiter_key_for_unknown_method(): void
    {
        $user = User::factory()->create();
        $currency = Currency::create(['symbol' => 'EUR', 'main' => true, 'user_id' => $user->id]);

        $valuation = Valuation::create([
            'name' => 'TEST',
            'currency_id' => $currency->id,
            'update_method' => ValuationUpdateMethod::FIXED,
            'user_id' => $user->id,
        ]);

        $this->assertEquals('none', $valuation->rate_limiter_key);
    }

    public function test_last_update_relationship(): void
    {
        $user = User::factory()->create();
        $currency = Currency::create(['symbol' => 'EUR', 'main' => true, 'user_id' => $user->id]);

        $valuation = Valuation::create([
            'name' => 'TEST',
            'currency_id' => $currency->id,
            'update_method' => ValuationUpdateMethod::FIXED,
            'user_id' => $user->id,
        ]);

        // Create older update
        CotationUpdate::create([
            'cotation_id' => $valuation->id,
            'date' => now()->subDays(2),
            'value' => 100.0,
            'status' => 'success',
            'user_id' => $user->id,
        ]);

        // Create newer update
        $newerUpdate = CotationUpdate::create([
            'cotation_id' => $valuation->id,
            'date' => now(),
            'value' => 110.0,
            'status' => 'success',
            'user_id' => $user->id,
        ]);

        $this->assertEquals($newerUpdate->id, $valuation->lastUpdate()->id);
    }

    public function test_get_update_periodicity_attribute(): void
    {
        $user = User::factory()->create();
        $this->actingAs($user);

        $currency = Currency::create(['symbol' => 'EUR', 'main' => true, 'user_id' => $user->id]);

        $valuation = Valuation::create([
            'name' => 'TEST',
            'currency_id' => $currency->id,
            'update_method' => ValuationUpdateMethod::YAHOO,
            'user_id' => $user->id,
        ]);

        $periodicity = $valuation->update_periodicity;

        $this->assertIsArray($periodicity);
        // With new Settings framework, should return default values
        $this->assertSame(['value' => 6, 'type' => 'hour'], $periodicity);
    }

    public function test_get_update_periodicity_text_attribute(): void
    {
        $user = User::factory()->create();
        $this->actingAs($user);

        $currency = Currency::create(['symbol' => 'EUR', 'main' => true, 'user_id' => $user->id]);

        $valuation = Valuation::create([
            'name' => 'TEST',
            'currency_id' => $currency->id,
            'update_method' => ValuationUpdateMethod::YAHOO,
            'user_id' => $user->id,
        ]);

        $periodicityText = $valuation->update_periodicity_text;

        $this->assertIsString($periodicityText);
        // With new Settings framework, should return default periodicity text
        $this->assertEquals('every 6 hour(s)', $periodicityText);
    }
}
