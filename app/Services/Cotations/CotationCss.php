...
class CotationCss implements CotationInterface
{
    ...
    public function getQuote(): float
    {
        try {
            ...
            $selector = $this->cotation->update_data['selector'] ?? '';

            if (empty($url)) {
                throw new CotationException(
                    $this->cotation,
                    'URL is required for CSS-based cotation',
                    null,
                    'Missing field: url'
                );
            }
            ...
            if (empty($selector)) {
                throw new CotationException(
                    $this->cotation,
                    'CSS selector is required for CSS-based cotation',
                    null,
